...
for sec in sec_list:
    ...
    for dir in dirs:
        ...
        # 中心点の軌跡から新たにグループを作成
        cloud_folder_path = f"/Users/kai/大学/小川研/LiDAR_step_length/20241025/pcd_0025s/{pcd_info_list.dir_name}"
        ...
        for group_idx in range(len(move_flg_list)):
            ...
            if move_flg:
                ...
                ax = ax_set.set_ax(ax, title=f"{pcd_info_list.dir_name}_speed", xlabel="time", ylabel="speed", xlim=[0, max(time_idx_list)], ylim=[min(speed_conv_list)-2, max(speed_conv_list)+2])
                ...
                fig = plt.figure(figsize=(10, 5))
                ax = fig.add_subplot(111)
                ax = ax_set.set_ax(ax, title=f"{pcd_info_list.dir_name}_step", xlim=[1000, 11000], ylim=[-500, 500])
