...
# ごみ取りを行う
sec_list = ["01"]
noize_folder = "/Users/kai/大学/小川研/LiDAR_step_length/20241204/pcd_01s/3d/cose_1_0_f_1204"

noize_cloud, noize_points = get_noize_points(noize_folder)

for sec in sec_list:
    active_folder_list = glob.glob(f"/Users/kai/大学/小川研/LiDAR_step_length/20241204/pcd_{sec}s/3d/*")
    for active_folder in active_folder_list:
        ...
        for cloud in tqdm(pcd_info_list.cloud_list):
            ...
            if new_cloud is None:
                ...
                ax1.scatter(cloud[:, 0], cloud[:, 1], cloud[:, 2], s=1)
                ax2.scatter(new_cloud[:, 0], new_cloud[:, 1], new_cloud[:, 2], s=1)
